...
// 타임아웃 설정 - [[memory:5039126]]에 따라 확장
...
function doPost(e) {
  try {
    ...
    // 🎯 NEW: AI 역량진단 시스템 처리
    if (requestData.formType === 'ai-capability-diagnosis') {
      console.log('🚀 AI 역량진단 시스템 처리 시작');
...
function doGet(e) {
  try {
    ...
    // AI 역량진단 관련 파라미터 처리
    if (e.parameter) {
      const { diagnosisId, path } = e.parameter;
      
      // AI 역량진단 상태 조회
      if (path === 'status' && diagnosisId && diagnosisId.startsWith('ACD-')) {
        console.log('📊 AI 역량진단 상태 조회:', diagnosisId);
        return handleGetAICapabilityDiagnosisStatus(diagnosisId);
      }
      
      // AI 역량진단 결과 조회
      if (diagnosisId && diagnosisId.startsWith('ACD-')) {
        console.log('📊 AI 역량진단 결과 조회:', diagnosisId);
...
function performFreeDiagnosisAIAnalysis(diagnosisId, data) {
  ...
  try {
    ...
    updateDiagnosisProgress(diagnosisId, 'AI분석중', '데이터를 심층 분석하고 있습니다 (예상 소요시간: 10-15분)');
      ...
      try {
        ...
        if (retryCount < maxRetries) {
          ...
          updateDiagnosisProgress(diagnosisId, 'AI분석중', 
            `분석 재시도 중... (${retryCount}/${maxRetries}) - ${apiError.message || '일시적 오류'}`);
          
          if (retryCount < maxRetries) {
            Utilities.sleep(30000); // 30초 대기 (더 긴 대기시간)
          }
        }
    }
    ...
    // 경과 시간 체크
    const elapsedMinutes = Math.floor((new Date() - analysisStartTime) / 60000);
    console.log(`⏱️ 현재까지 소요시간: ${elapsedMinutes}분`);
    ...
    try {
      ...
      // 이메일 발송 성공 후에만 완료 상태로 업데이트
      updateDiagnosisProgress(diagnosisId, '완료', '보고서 발송 완료');
      
      // 총 소요시간 계산
      const totalMinutes = Math.floor((new Date() - analysisStartTime) / 60000);
      console.log(`✅ 전체 프로세스 완료 - 총 소요시간: ${totalMinutes}분`);
    ...
    console.error('오류 발생 단계:', currentStep);
    
    // 진행 상태를 오류로 업데이트
    updateDiagnosisProgress(diagnosisId, '오류발생', `${currentStep} 단계에서 오류 발생: ${error.message}`);
...
function handleAICapabilityDiagnosisSubmission(data) {
  try {
    ...
    // 확인 이메일 발송 (실제 업종명 포함)
    const actualIndustry = getActualIndustryName(data.industry, data.customIndustry);
    sendAICapabilityDiagnosisConfirmationEmail(data.email, data.companyName, diagnosisId, actualIndustry);
    
    // 관리자 알림 (실제 업종명 포함)
    sendAICapabilityDiagnosisAdminNotification(data, diagnosisId, actualIndustry);
...
function saveAICapabilityDiagnosisApplication(diagnosisId, data, timestamp) {
  ...
  // 업종 정보 처리 (기타 업종 직접입력 반영)
  const industryInfo = getActualIndustryName(data.industry, data.customIndustry);
...
/**
 * 실제 업종명 가져오기 (기타 업종 직접입력 처리)
 */
function getActualIndustryName(industry, customIndustry) {
 ...
 * AI 역량진단 확인 이메일 발송
 */
function sendAICapabilityDiagnosisConfirmationEmail(email, companyName, diagnosisId, industry) {
 ...
 * AI 역량진단 관리자 알림
 */
function sendAICapabilityDiagnosisAdminNotification(data, diagnosisId, industry) {
...
function performAICapabilityDiagnosisAnalysis(diagnosisId, data) {
  try {
    // 트리거에서 호출된 경우 data를 PropertiesService에서 가져오기
    if (!data && diagnosisId) {
      const storedData = PropertiesService.getScriptProperties().getProperty(`diagnosis_${diagnosisId}`);
      if (storedData) {
...
`;
  ...
  try {
    ...
    const report = {
      diagnosisId: data.diagnosisId,
      companyName: data.companyName,
      industry: actualIndustry, // 실제 업종 정보 포함 (기타 업종 직접입력 반영)
 ...
 * AI 역량진단 결과 이메일 발송
 */
function sendAICapabilityDiagnosisResultEmail(email, companyName, diagnosisId, result, industry) {
