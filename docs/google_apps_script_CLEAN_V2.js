...
const CONFIG = {
  ...
  function generateAIReportAsync(diagnosisId, data, aiScores, benchmarkData) {
    try {
      updateProgressStatus(diagnosisId, 70, 'AI 분석 보고서 생성 중');
      
      // GEMINI AI 보고서 생성
      const aiReport = generateAIReport(data, aiScores, benchmarkData);
      ...
      console.log('✅ AI 보고서 생성 완료:', diagnosisId);
   ...
   * GEMINI AI 보고서 생성
   */
  function generateAIReport(data, aiScores, benchmarkData) {
    ...
    try {
      ...
      // Fallback 보고서
      return generateFallbackReport(data, aiScores, benchmarkData);
    }
  }
  ...
  **장기 (6-12개월):**
  ...
  function saveToGoogleSheets(diagnosisId, data, aiScores, benchmarkData) {
    ...
    try {
      ...
      console.log('✅ 확장된 Google Sheets 저장 완료 (58개 컬럼)');
      return diagnosisId;
  ...
  function saveFreeDiagnosisApplication(diagnosisId, data, timestamp) {
    ...
    try {
      ...
      // 확장된 저장 함수 호출
      return saveToGoogleSheets(diagnosisId, data, aiScores, benchmarkData);
      
    } catch (error) {
      console.error('❌ 무료 진단 신청 데이터 저장 오류:', error);
      throw error;
    }
  }
